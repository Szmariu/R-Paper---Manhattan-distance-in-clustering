...
It doesn't seem like that's the case - the outliers are scattered quite evenly across the time. I wasn't able to find any information on this - maybe it is caused by a bug in the dist() function, or an background app taking the focus of the processor for a couple of milliseconds (e.g. update software, or cloud syncing apps).
...
Strange outliers can be observed, similar to these in distance matrices. After a short analisys of the CClust source code, I was able to find that it indeed uses the dist() function, which may have caused the outliers.  
...
Below, we can see a sample clustering done in clara. Number of samples, and the sample size was increased from defults, as is suggested in the documentation.   
The silhouette plot indicates that the resulting clustering is quite good, with a average silhouette width ot 0.82. 
...
Simmilarly to CClust, the Clara supports two distance metrics - euclidean and manhattan, the former beeing the default. Since Clara it is noticabely faster than CClust, this test was run 600 times for each metric. Overall it took around 10 minutes to complete. 
...
This time the results are more simmilar to thoes from the syntetic benchmark. When using the euclidean distance instead of manhattan in the clara algorithm, we can expect it to be around **`r sprintf("%1.2f%%", (timeClaraResults[2,5] - timeClaraResults[1,5]) / timeClaraResults[1,5]*100) `** slower. 
...
Like in previous experiments, the test results are statistically significant. 
